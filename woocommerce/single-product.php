...
(function () {
  ...
  document.querySelectorAll('.lumea-pdp-thumb').forEach(function (btn) {
    btn.addEventListener('click', function () {
      document.querySelectorAll('.lumea-pdp-thumb').forEach(function (b) { b.classList.remove('is-active'); });
      btn.classList.add('is-active');
      if (!mainImg) return;
      mainImg.style.opacity = '0';
      mainImg.style.transform = 'scale(0.97)';
      setTimeout(function () {
        /* Clear srcset/sizes so browser doesn't override src with responsive variants */
  ...
  if (qtyInput) {
    document.querySelector('.lumea-qty-minus') && document.querySelector('.lumea-qty-minus').addEventListener('click', function () {
      var v = parseInt(qtyInput.value, 10); var min = parseInt(qtyInput.min, 10) || 1;
      if (v > min) { qtyInput.value = v - 1; qtyInput.dispatchEvent(new Event('change')); }
    });
    document.querySelector('.lumea-qty-plus') && document.querySelector('.lumea-qty-plus').addEventListener('click', function () {
      var v = parseInt(qtyInput.value, 10); var max = parseInt(qtyInput.max, 10) || 999;
