<?php
/**
 * Custom content for a single product.
 *
...
<main class="lumea-pdp" id="lumeaPage">
		...
		<div class="lumea-pdp-hero-inner">
			...
			<div class="lumea-pdp-gallery">
				...
				<div class="lumea-pdp-thumbs">
					...
					foreach ( $all_image_ids as $i => $img_id ) :
						...
						$alt   = $alt ? $alt : $name;
						?>
					<button type="button" class="lumea-pdp-thumb<?php echo esc_attr( 0 === $i ? ' is-active' : '' ); ?>"
				...
				<div class="lumea-pdp-img-wrap">
					...
					<button type="button" class="lumea-pdp-zoom-btn" id="lumeaZoomBtn" aria-label="<?php esc_attr_e( 'Zoom image', 'lumea' ); ?>">
						<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
			...
			<div class="lumea-pdp-info" id="lumeaPdpInfo">
				...
				<div class="lumea-pdp-meta-top">
					...
					<?php elseif ( null !== $stock_qty && 0 < $stock_qty && 5 >= $stock_qty ) : ?>
				...
				<div class="lumea-pdp-rating">
					<div class="lumea-pdp-stars" aria-label="<?php /* translators: %.1f: product rating (e.g. 4.5) */ echo esc_attr( sprintf( __( 'Rated %.1f out of 5', 'lumea' ), $average ) ); ?>">
						<?php
						for ( $star_index = 1; 5 >= $star_index; $star_index++ ) :
							$fill_pct = max( 0, min( 100, ( (float) $average - ( $star_index - 1 ) ) * 100 ) );
							$gid      = 'sg-' . $product_id . '-' . $star_index;
							?>
						<svg width="14" height="14" viewBox="0 0 24 24" aria-hidden="true">
							<defs><linearGradient id="<?php echo esc_attr( $gid ); ?>" x1="0" x2="1" y1="0" y2="0">
								<stop offset="<?php echo esc_attr( $fill_pct ); ?>%" stop-color="#8f4d43"/>
					...
					<a href="#lumea-reviews" class="lumea-pdp-rating-link">
					<?php
					/* translators: %s: formatted number of reviews */
					echo esc_html( sprintf( _n( '%s review', '%s reviews', $rating_count, 'lumea' ), number_format_i18n( $rating_count ) ) );
					?>
				</a>
				</div>
				<?php endif; ?>
				...
				<div class="lumea-pdp-atc">
					...
					<div class="lumea-atc-btn-row">
						...
						<div class="lumea-card-actions">
							<div class="lumea-card-atc-wrap<?php echo esc_attr( $in_cart ? ' is-added' : '' ); ?>">
								<a href="<?php echo esc_url( $atc_url ); ?>"
									...
									echo esc_attr( sprintf( __( 'Add %s to bag', 'lumea' ), $name ) );
									?>
									">
							...
							<a href="<?php echo esc_url( $cart_url ); ?>"
								class="lumea-view-cart-btn<?php echo esc_attr( $in_cart ? ' is-active' : '' ); ?>"
	...
	<div class="lumea-pdp-details">
		<div class="lumea-pdp-details-inner">
			...
			<div class="lumea-pdp-acc" data-acc>
				<button type="button" class="lumea-pdp-acc-trigger is-open" aria-expanded="true" data-acc-trigger>
			...
			<div class="lumea-pdp-acc" data-acc>
				<button type="button" class="lumea-pdp-acc-trigger" aria-expanded="false" data-acc-trigger>
			...
			<div class="lumea-pdp-acc" data-acc>
				...
				<div class="lumea-pdp-acc-body" data-acc-body>
					<div class="lumea-pdp-acc-content">
						...
						if ( ! $how ) {
							$how = $product->get_attribute( 'pa_how_to_use' );
						}
						$how_html = $how
							? $how
				...
				<div class="lumea-pdp-rev-score">
					...
					<div class="lumea-pdp-rev-meta">
						<div class="lumea-pdp-stars">
							<?php for ( $star_index = 1; 5 >= $star_index; $star_index++ ) : ?>
							<svg width="16" height="16" viewBox="0 0 24 24" fill="<?php echo esc_attr( round( $average ) >= $star_index ? '#aa6258' : '#e5dbd8' ); ?>" aria-hidden="true"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
							<?php endfor; ?>
						</div>
						<p>
						<?php
						/* translators: %s: formatted number of reviews */
						echo esc_html( sprintf( _n( 'Based on %s review', 'Based on %s reviews', $rating_count, 'lumea' ), number_format_i18n( $rating_count ) ) );
						?>
					</p>
					</div>
				</div>
...
<!-- Lightbox -->
<div class="lumea-lightbox" id="lumeaLightbox" role="dialog" aria-modal="true" aria-label="<?php esc_attr_e( 'Image zoom', 'lumea' ); ?>" aria-hidden="true" inert>
	<button type="button" class="lumea-lightbox-close" id="lumeaLightboxClose" aria-label="<?php esc_attr_e( 'Close', 'lumea' ); ?>">
