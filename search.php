	...
	<section class="lumea-search-hero">
		<div class="lumea-search-hero-inner">
			...
			<h1 class="lumea-search-title">
				<?php echo esc_html( $query_term ? sprintf( '“%s”', $query_term ) : __( 'Search', 'lumea' ) ); ?>
			</h1>
	...
	<div class="lumea-search-body">
		<div class="lumea-search-body-inner">
			...
			<div class="lumea-search-filters" data-lumea-search-filters>
				<button type="button" class="lumea-search-tab is-active" data-filter="all">
					<?php esc_html_e( 'All', 'lumea' ); ?>
					<span class="lumea-search-tab-count"><?php echo esc_html( number_format_i18n( $count ) ); ?></span>
				</button>
				<?php if ( $product_count ) : ?>
				<button type="button" class="lumea-search-tab" data-filter="product">
					<?php esc_html_e( 'Products', 'lumea' ); ?>
					<span class="lumea-search-tab-count"><?php echo esc_html( number_format_i18n( $product_count ) ); ?></span>
				</button>
				<?php endif; ?>
				<?php if ( $post_count ) : ?>
				<button type="button" class="lumea-search-tab" data-filter="post">
					<?php esc_html_e( 'Articles', 'lumea' ); ?>
					<span class="lumea-search-tab-count"><?php echo esc_html( number_format_i18n( $post_count ) ); ?></span>
				</button>
				<?php endif; ?>
			</div>
			...
			<div class="lumea-search-grid" data-lumea-search-grid>
				...
				while ( have_posts() ) :
					the_post();
					$lumea_post_type = get_post_type();
					$is_product      = ( 'product' === $lumea_post_type );
					$is_post         = ( 'post' === $lumea_post_type );
				...
				>
					...
					<div class="lumea-search-card-body">
						<?php if ( $cat_name ) : ?>
						<p class="lumea-search-card-cat"><?php echo esc_html( $cat_name ); ?></p>
						<?php endif; ?>
						...
						<p class="lumea-search-card-excerpt">
							<?php echo wp_kses_post( wp_trim_words( get_the_excerpt(), 18, '&hellip;' ) ); ?>
						</p>
			...
			<div class="lumea-search-empty">
				<div class="lumea-search-empty-icon" aria-hidden="true">
					<svg width="2.25rem" height="2.25rem" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
				</div>
