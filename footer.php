...
$footer_headline        = get_theme_mod( 'lumea_footer_headline', __( 'Discover your skin’s new ritual. Start today.', 'lumea' ) );
$footer_cta_text        = get_theme_mod( 'lumea_footer_cta_text', __( 'Shop Collection', 'lumea' ) );
$footer_copy            = get_theme_mod( 'lumea_footer_copy', __( 'Luméa · Botanical Skincare', 'lumea' ) );
$footer_connect_heading = get_theme_mod( 'lumea_footer_connect_heading', __( 'Connect', 'lumea' ) );
	...
	<div class="lumea-footer-main">
		<div class="lumea-footer-inner">
			...
			<div class="lumea-footer-col lumea-footer-col--cta">
				...
				<p class="lumea-footer-copy">&copy; <?php echo esc_html( wp_date( 'Y' ) ); ?> <?php echo esc_html( $footer_copy ); ?></p>
			</div>
			...
			<div class="lumea-footer-col lumea-footer-col--connect">
				<h2 class="lumea-footer-connect-h"><?php echo esc_html( $footer_connect_heading ); ?></h2>
				<!-- Social icons always visible directly below Connect heading -->
				<div class="lumea-footer-social">
					<a href="<?php echo esc_url( $footer_ig ? $footer_ig : '#' ); ?>" aria-label="<?php esc_attr_e( 'Instagram', 'lumea' ); ?>"
					<?php
					if ( $footer_ig ) :
						?>
					...
					<a href="<?php echo esc_url( $footer_tk ? $footer_tk : '#' ); ?>" aria-label="<?php esc_attr_e( 'TikTok', 'lumea' ); ?>"
					<?php
					if ( $footer_tk ) :
						?>
						target="_blank" rel="noopener noreferrer"
					...
					<a href="<?php echo esc_url( $footer_pin ? $footer_pin : '#' ); ?>" aria-label="<?php esc_attr_e( 'Pinterest', 'lumea' ); ?>"
					<?php
					if ( $footer_pin ) :
						?>
						target="_blank" rel="noopener noreferrer"
				...
				<?php
				if ( is_active_sidebar( 'lumea-footer' ) ) :
					?>
				<aside class="lumea-footer-widgets widget-area" aria-label="<?php esc_attr_e( 'Footer widgets', 'lumea' ); ?>">
					<?php dynamic_sidebar( 'lumea-footer' ); ?>
				</aside>
				<?php endif; ?>
			</div>
		</div>
