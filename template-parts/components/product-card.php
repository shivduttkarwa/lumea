...
$hover_image     = (string) $data['hover_image'];
...
<article class="<?php echo esc_attr( $card_class ); ?>">
	...
	<a href="<?php echo esc_url( $product_url ); ?>" class="lumea-lp-media">
		<?php if ( $badge ) : ?>
		<span class="lumea-lp-badge<?php echo esc_attr( $is_sale ? ' lumea-lp-badge--sale' : '' ); ?>"><?php echo esc_html( $badge ); ?></span>
		<?php endif; ?>
		<?php if ( $main_image ) : ?>
		...
		<?php else : ?>
			...
			echo wp_kses_post(
				wc_placeholder_img(
					...
					array(
						'class' => 'lumea-lp-img lumea-lp-img--main',
						'alt'   => $product_name,
					)
				)
			);
	...
	<div class="lumea-lp-body">
		...
		<div class="lumea-lp-title-row">
			<h3 class="lumea-lp-name"><a href="<?php echo esc_url( $product_url ); ?>"><?php echo esc_html( $product_name ); ?></a></h3>
			<?php if ( $product_id ) : ?>
		...
		<div class="lumea-lp-pricing">
			...
			<?php endif; ?>
			<span class="lumea-lp-price<?php echo esc_attr( $is_sale ? ' lumea-lp-price--sale' : '' ); ?>"><?php echo wp_kses_post( $price_html ); ?></span>
		</div>
